...
def evaluation(data,result,label):#注输入向量均为numpy.array类型
    ...
    for i in range(data.shape[0]):
        m=m+sum(data[i,:])
    m=m/2
    #计算模块度，采用Modularity的矩阵形式计算 https://blog.csdn.net/marywbrown/article/details/62059231?utm_source=blogxgwz8
    S = np.zeros((data.shape[0],len(np.unique(result[0,:]))))
    for i in range(data.shape[1]):
        S[i,result[0,i]]=1
    B=np.zeros((data.shape[0],data.shape[1]))
    for i in range(data.shape[0]):
        for j in range(data.shape[1]):
            B[i,j]=data[i,j]-np.sum(data[i,:])*np.sum(data[j,:])/(2*m)
    Q=0.5*np.trace(np.dot(np.dot(S.transpose(),B),S))/m
    '''
    主要计算各种度量指标
    '''
    if len(label)!=0:#label为不为空
        ...
        Hubert=(comb(data.shape[0],2)*TP-(TP+FN)*(TP+FP))/math.sqrt((TP+FN)*(TP+FP)*(TN+FN)*(TN+FP))#计算的是Hubert指标[-1,1],值越大越好,-1表示完全不一致
        ...
        return FM,ARI,Phi,Hubert,K,RT,precision,recall,F1,NMI,Q#返回计算的结果
    else:
        return Q
